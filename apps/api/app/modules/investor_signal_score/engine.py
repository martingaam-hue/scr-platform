...
class InvestorSignalScoreEngine:
    ...
    async def calculate(self) -> EngineResult:
        ...
        overall = sum(dim.score * DIMENSION_WEIGHTS[key] for key, dim in dimensions.items())
    ...
    def _score_financial_capacity(self, ctx: dict) -> DimensionResult:
        ...
        c = CriterionResult(
            "AUM Documentation",
            "Current AUM recorded in portfolio",
            20 if has_aum else 0,
            20,
            has_aum,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Fund Size Evidence",
            "Target AUM / fund size documented",
            15 if has_target else 0,
            15,
            has_target,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Deployment History",
            "At least one active portfolio holding",
            15 if has_holdings else 0,
            15,
            has_holdings,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "Target AUM exceeds current (reserve capacity visible)",
            10 if has_reserve else 0,
            10,
            has_reserve,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "3+ portfolio holdings (demonstrates follow-on activity)",
            10 if has_followon else 0,
            10,
            has_followon,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Return Targets (LP Commitment)",
            "Target IRR specified in mandate",
            15 if has_irr else 0,
            15,
            has_irr,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "5+ holdings indicate active capital deployment",
            15 if has_cap_calls else 0,
            15,
            has_cap_calls,
        )
        criteria.append(c)
    ...
    def _score_risk_management(self, ctx: dict) -> DimensionResult:
        ...
        c = CriterionResult(
            "Written Risk Policy",
            "At least one risk assessment created",
            20 if has_risk_policy else 0,
            20,
            has_risk_policy,
        )
        criteria.append(c)
        ...
        has_insurance = any(
            getattr(r, "output_data", None)
            and isinstance(r.output_data, dict)
            and "insurance" in str(r.output_data).lower()
            for r in risk_assessments
        )
        c = CriterionResult(
            "Insurance Portfolio",
            "Insurance coverage documented in risk assessment",
            15 if has_insurance else 0,
            15,
            has_insurance,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "Multiple risk assessments (indicates risk strategy breadth)",
            15 if has_hedging else 0,
            15,
            has_hedging,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "Holdings span 2+ asset types",
            15 if has_diversification else 0,
            15,
            has_diversification,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "Risk assessments suggest governance process",
            10 if has_risk_committee else 0,
            10,
            has_risk_committee,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Stress Test Results",
            "Scenario analysis found in risk assessment output",
            15 if has_stress else 0,
            15,
            has_stress,
        )
        criteria.append(c)
        ...
        has_compliance = any(
            p.sfdr_classification != SFDRClassification.NOT_APPLICABLE for p in portfolios
        )
        c = CriterionResult(
            "Compliance Framework",
            "SFDR classification applied to portfolio",
            10 if has_compliance else 0,
            10,
            has_compliance,
        )
        criteria.append(c)
    ...
    def _score_investment_strategy(self, ctx: dict) -> DimensionResult:
        ...
        c = CriterionResult(
            ...
            "Mandate has a descriptive name (thesis proxy)",
            20 if has_thesis else 0,
            20,
            has_thesis,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "Both sectors and stages specified in mandate",
            15 if has_spec else 0,
            15,
            has_spec,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "At least one exited investment (proves track record)",
            20 if has_track else 0,
            20,
            has_track,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "2+ sectors defined in mandate",
            10 if has_sector_focus else 0,
            10,
            has_sector_focus,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Stage Preference Definition",
            "Investment stage preference specified",
            10 if has_stages else 0,
            10,
            has_stages,
        )
        criteria.append(c)
        ...
        has_geo = len(geos) >= 2
        c = CriterionResult(
            "Geographic Scope (Co-investment Framework)",
            "2+ geographies defined — signals co-investment readiness",
            10 if has_geo else 0,
            10,
            has_geo,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "Portfolio strategy type defined",
            15 if has_construction else 0,
            15,
            has_construction,
        )
        criteria.append(c)
    ...
    def _score_team_experience(self, ctx: dict) -> DimensionResult:
        ...
        c = CriterionResult(
            "Team Members",
            "2+ users registered in the organisation",
            15 if has_team else 0,
            15,
            has_team,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Investment Experience Evidence",
            "Mandate exists — signals prior investment experience",
            15 if has_experience else 0,
            15,
            has_experience,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Sector Expertise Evidence",
            "3+ distinct sectors across all mandates",
            15 if has_sector_expertise else 0,
            15,
            has_sector_expertise,
        )
        criteria.append(c)
        pts += c.points
        ...
        c = CriterionResult(
            "Board Position Track Record",
            "2+ holdings (implies governance participation)",
        ...
        c = CriterionResult(
            ...
            "3+ portfolio holdings (demonstrates active deal sourcing)",
            15 if has_deals else 0,
            15,
            has_deals,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Team References",
            "3+ team members (larger teams typically have testimonials)",
        ...
        c = CriterionResult(
            ...
            "4+ users — sufficient team size for structure documentation",
            10 if has_org_chart else 0,
            10,
            has_org_chart,
        )
        criteria.append(c)
        ...
        has_key_person = any(getattr(u, "role", None) == UserRole.ADMIN for u in users)
        c = CriterionResult(
            "Key Person Identification",
            "Admin role assigned — identifies key decision maker",
            10 if has_key_person else 0,
            10,
            has_key_person,
        )
        criteria.append(c)
    ...
    def _score_esg_commitment(self, ctx: dict) -> DimensionResult:
        ...
        c = CriterionResult(
            "ESG Policy Document",
            "ESG requirements defined in mandate",
            20 if has_esg_policy else 0,
            20,
            has_esg_policy,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "SFDR Classification",
            "At least one portfolio classified under SFDR Article 6/8/9",
            15 if has_sfdr else 0,
            15,
            has_sfdr,
        )
        criteria.append(c)
        ...
        has_impact = any(
            getattr(p, "sfdr_classification", None)
            in (SFDRClassification.ARTICLE_8, SFDRClassification.ARTICLE_9)
            for p in portfolios
        )
        c = CriterionResult(
            "Impact Measurement Framework",
            "SFDR Article 8/9 classification implies impact measurement",
            20 if has_impact else 0,
            20,
            has_impact,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "UN PRI Membership",
            "PRI / UN Principles referenced in ESG requirements",
            15 if has_pri else 0,
            15,
            has_pri,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            ...
            "SDG goals referenced in ESG requirements",
            15 if has_sdg else 0,
            15,
            has_sdg,
        )
        criteria.append(c)
        ...
        if not has_esg_history:
            ...
            has_esg_history = (
                bool(exclusions) and isinstance(exclusions, dict) and len(exclusions) > 0
            )
        c = CriterionResult(
            "ESG Reporting History",
            "ESG metrics in portfolio or exclusion criteria set",
            15 if has_esg_history else 0,
            15,
            has_esg_history,
        )
        criteria.append(c)
    ...
    def _score_platform_readiness(self, ctx: dict) -> DimensionResult:
        ...
        has_complete_profile = bool(getattr(org, "name", "")) and bool(
            getattr(org, "logo_url", None)
        )
        c = CriterionResult(
            "Profile Complete",
            "Organisation has name and logo uploaded",
            20 if has_complete_profile else 0,
            20,
        ...
        c = CriterionResult(
            "Documents Uploaded",
            "5+ platform notifications (proxy for document activity)",
            15 if has_docs else 0,
            15,
            has_docs,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Mandate Fully Specified",
            "Active mandate with all required fields populated",
            15 if has_full_mandate else 0,
            15,
            has_full_mandate,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "3+ Investor Personas",
            "3 or more investor personas created",
            10 if has_personas else 0,
            10,
            has_personas,
        )
        criteria.append(c)
        ...
        c = CriterionResult(
            "Deal Engagement",
            "Active engagement with matching recommendations",
            15 if has_engagement else 0,
            15,
            has_engagement,
        )
        criteria.append(c)
        ...
        active_matches = [
            ...
            in (MatchStatus.INTERESTED, MatchStatus.ENGAGED, MatchStatus.INTRO_REQUESTED)
        ]
        has_responses = len(active_matches) >= 1
        c = CriterionResult(
            "Match Response Rate",
            "Responded to at least one match (interested/engaged)",
            10 if has_responses else 0,
            10,
            has_responses,
        )
        criteria.append(c)
        ...
        for dim_key, dim in dimensions.items():
            ...
            for crit in dim.criteria:
                if not crit.met:
                    ...
                    effort = effort_map.get(crit.max_points, "medium")
                    actions.append(
                        ImprovementAction(
                            title=crit.name,
    ...
    async def calculate_deal_alignment(self, project_id: uuid.UUID) -> dict[str, Any]:
        ...
        from app.models.projects import SignalScore as ProjectSignalScore
        ...
        tick_max = (
            float(getattr(best_mandate, "ticket_size_max", float("inf")) or float("inf"))
            if best_mandate
            else float("inf")
        )
        ...
        mandate_risk = (
            getattr(best_mandate, "risk_tolerance", RiskTolerance.MODERATE)
            if best_mandate
        ...
        if mandate_irr > 0:
            irr_ratio = (
                min(project_irr / mandate_irr, mandate_irr / project_irr)
                if project_irr > 0
                else 0.4
            )
    ...
    async def _get_org(self):
        ...
        result = await self.db.execute(select(Organization).where(Organization.id == self.org_id))
    ...
    async def _get_personas(self):
        ...
        result = await self.db.execute(
            select(InvestorPersona).where(
                InvestorPersona.org_id == self.org_id, InvestorPersona.is_deleted.is_(False)
            )
        )
        return list(result.scalars().all())
