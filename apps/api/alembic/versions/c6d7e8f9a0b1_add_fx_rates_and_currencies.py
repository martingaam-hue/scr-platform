...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_fx_pair_date_lookup", "fx_rates", ["base_currency", "quote_currency", "rate_date"]
    )

    # Add currency columns to existing tables
    op.add_column(
        "projects", sa.Column("project_currency", sa.String(3), server_default="EUR", nullable=True)
    )
