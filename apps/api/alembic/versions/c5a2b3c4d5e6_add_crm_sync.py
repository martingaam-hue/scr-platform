...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "sync_direction", sa.String(length=20), server_default="bidirectional", nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "connection_id", "scr_entity_type", "scr_entity_id", name="uq_crm_entity_mapping_scr"
        ),
    )
    op.create_index(
        "ix_crm_entity_mappings_connection_id", "crm_entity_mappings", ["connection_id"]
    )
