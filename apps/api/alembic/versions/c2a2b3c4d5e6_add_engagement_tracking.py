...
down_revision: str | tuple[str, ...] = ("aa1122334455", "f1a2b3c4d5e6")
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_document_engagements_doc_opened", "document_engagements", ["document_id", "opened_at"]
    )
    op.create_index(
        "ix_document_engagements_user_org", "document_engagements", ["user_id", "org_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "deal_room_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("deal_rooms.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column(
            "investor_org_id",
    ...
    op.create_index(
        "ix_deal_engagement_summaries_project_id", "deal_engagement_summaries", ["project_id"]
    )
    op.create_index(
        "ix_deal_engagement_summaries_project_investor",
        "deal_engagement_summaries",
        ["project_id", "investor_org_id"],
    )


def downgrade() -> None:
    op.drop_index(
        "ix_deal_engagement_summaries_project_investor", table_name="deal_engagement_summaries"
    )
