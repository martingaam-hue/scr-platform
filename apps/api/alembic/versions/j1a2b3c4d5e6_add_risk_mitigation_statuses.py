...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "evidence_document_ids", postgresql.ARRAY(postgresql.UUID(as_uuid=True)), nullable=True
        ),
    ...
    op.create_index(
        "ix_risk_mitigation_statuses_project_id", "risk_mitigation_statuses", ["project_id"]
    )
    op.create_index("ix_risk_mitigation_statuses_org_id", "risk_mitigation_statuses", ["org_id"])
    op.create_index(
        "ix_risk_mitigation_statuses_risk_item_id", "risk_mitigation_statuses", ["risk_item_id"]
    )
