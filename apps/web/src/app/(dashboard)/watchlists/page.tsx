  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500 mt-1">AI-powered deal discovery and monitoring</p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Watchlists */}
        <div>
          <h2 className="font-semibold text-neutral-900 mb-3">My Watchlists</h2>
          <div className="space-y-3">
            {watchlists.map((wl) => {
              const Icon = WATCH_TYPE_ICON[wl.watch_type] ?? Bell
              return (
                <div key={wl.id} className={`rounded-xl border bg-white p-4 ${wl.is_active ? "border-neutral-200" : "border-gray-100 opacity-60"}`}>
                  <div className="flex items-center gap-3">
                    <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${wl.is_active ? "bg-primary-100" : "bg-neutral-100"}`}>
                      <Icon className={`h-5 w-5 ${wl.is_active ? "text-primary-600" : "text-neutral-400"}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-neutral-900 text-sm">{wl.name}</p>
                  ...
                  <div className="flex items-center gap-2 mt-2 pl-12">
                    {wl.alert_channels.map(ch => (
                      <span key={ch} className="px-2 py-0.5 rounded-full text-xs bg-neutral-100 text-neutral-600 capitalize">
                        {ch.replace(/_/g, " ")}
                      </span>
                    ))}
                  </div>
            ...
            {watchlists.length === 0 && (
              <div className="text-center py-8 border border-dashed border-neutral-300 rounded-xl text-neutral-500">
                <BellOff className="h-8 w-8 text-neutral-300 mx-auto mb-2" />
        ...
        {/* Alert feed */}
        <div>
          <h2 className="font-semibold text-neutral-900 mb-3">Recent Alerts</h2>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-neutral-600 mt-0.5">{alert.watchlist_name}</p>
                    {alert.data?.title != null && (
                      <p className="text-xs text-neutral-500 mt-1">{String(alert.data.title)}</p>
                    )}
                    <p className="text-xs text-neutral-400 mt-1">{formatDate(alert.created_at)}</p>
                  </div>
                  {!alert.is_read && (
                    <button
                      onClick={() => markReadMutation.mutate(alert.id)}
                      className="flex-shrink-0 text-neutral-400 hover:text-green-500"
                      title="Mark as read"
                    >
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-neutral-900">New Watchlist</h2>
              <button onClick={() => setShowCreate(false)}><X className="h-5 w-5 text-neutral-400" /></button>
            </div>
            {/* NL Criteria Parser */}
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-1">Watch Type</label>
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-2">Alert Channels</label>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setShowCreate(false)} className="px-4 py-2 border border-neutral-300 rounded-lg text-sm">Cancel</button>
              <button
                onClick={() => {
                  let criteria = {}
